...
namespace MathFighter.Math.Questions
{
    ...
    public class DMQ_Derivatives : DynamicMathQuestion
    {
        ...
        public override string[] GetContent()
        {
            ...
            // create the question
            content[0] = "<i>f</i>(<i>x</i>) =#" + WrapMathString(FirstTermAsString(coefficient1, exponent1) + TermAsString(coefficient2, exponent2));

            // create the correct answer
            content[1] = "@<i>height</i>{0.8}#<i>f</i>'(<i>x</i>) =#" + WrapMathString(FirstTermAsString(coefficient1 * exponent1, exponent1 - 1) + TermAsString(coefficient2 * exponent2, exponent2 - 1));
            ...
            for (int i = 2; i < 5; i++)
            {
                ...
                do
                {
                    ...
                    pertAnswer = "@<i>height</i>{0.8}#<i>f</>'(<i>x</i>) =#" + WrapMathString(FirstTermAsString(newCoefficient1 * newExponent1, newExponent1 - 1) + TermAsString(newCoefficient2 * newExponent2, newExponent2 - 1));
        ...
        private string TermAsStringNoNeg(int coefficient, int exponent)
        {
            ...
            else if (exponent == 1)
            {
                ...
                {
                    return absCoefficient.ToString() + " * <i>x</i>";
                }
            }
            else
            {
                ...
                {
                    return absCoefficient.ToString() + " * <i>x</i> ^ " + exponent;
                }
            }
        }
    }

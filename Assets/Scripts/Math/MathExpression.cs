...
namespace MathFighter.Math
{
    ...
    public class MathExpression
    {
        ...
        private static MathOperatorExpressionRoot _expressionTree;
        ...
        public void ParseNewExpression(string expressionString)
        {
            ...
            if (tokens.Length == 1)
            {
                if(IsOperator(tokens[0]))
                    Debug.Log("String with only one token contains an operator as that sole token - should be a number");
        ...
        public static MathOperator CreateOperator(string token)
        {
            ...
            {
                Debug.Log("MathExpression::CreateOperator() - unrecognised operator string token: " + token);
                return null;
            }
        ...
        public LogicalRenderTexture GenerateTexture(float charHeight, float spacing, Font font, Vector4 backgroundColour)
        {
            ...
            if (backgroundColour.z != 0.0f)  // i.e. test for alpha being non-zero
            {
                ...
                // spriteBatch.Begin();
                lrt.Draw(expressionTexture.Texture, Vector2.zero, expressionTexture.Region, Color.white);
                // spriteBatch.End();

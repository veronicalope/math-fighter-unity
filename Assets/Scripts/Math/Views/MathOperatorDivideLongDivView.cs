...
namespace MathFighter.Math.Views
{
    ...
    {
        public MathOperatorDivideLongDivView(MathOperatorDivide op, float h, float spacing, Font font)
        {
        ...
        protected LogicalRenderTexture DrawOperatorToTexture(float barWidth, float h, Font font)
        {
            ...
            //// spriteBatch.Begin();
            lrt.Draw(leftCap.Texture, 
                new Rect(0, 0, leftCap.Width - trim * 2, leftCap.Height), 
                new Rect(leftCap.Region.x + trim, leftCap.Region.y, leftCap.Region.width - trim * 2, leftCap.Region.height), 
                Color.white);
            lrt.Draw(middle.Texture, 
                new Rect(leftCap.Width - trim * 2, 0, middleWidth, middle.Height), 
                new Rect(middle.Region.x + trim, middle.Region.y, middle.Region.width - trim * 2, middle.Region.height), 
                Color.white);
            lrt.Draw(rightCap.Texture, 
                new Rect(w - (rightCap.Width - trim * 2), 0, rightCap.Width - trim * 2, rightCap.Height), 
                new Rect(rightCap.Region.x + trim, rightCap.Region.y, rightCap.Region.width - trim * 2, rightCap.Region.height), 
                Color.white);
            //lrt.End();
        ...
        protected LogicalRenderTexture AssembleComponentTextures(float w, float h, float barWidth, LogicalRenderTexture opTexture, LogicalRenderTexture leftParamTexture, LogicalRenderTexture rightParamTexture)
        {
            ...
            // spriteBatch.Begin();
            lrt.Draw(opTexture.Texture, 
                new Vector2(rightParamTexture.Width, 0.0f), 
                opTexture.Region, 
                Color.white, 
                0.0f, 
                Vector2.zero, 
                1.0f, 
                0.5f);
            lrt.Draw(rightParamTexture.Texture, 
                new Vector2(0.0f, 0.0f), 
                leftParamTexture.Region, 
                Color.white, 
                0.0f, 
                Vector2.zero, 
                1.0f, 
                0.0f);
            lrt.Draw(leftParamTexture.Texture, 
                new Vector2((w - barWidth) + (barWidth - (float)leftParamTexture.Width) * 0.5f, 0.0f), 
                leftParamTexture.Region, 
                Color.white, 
                0.0f, 
                Vector2.zero, 
